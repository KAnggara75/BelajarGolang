...
# The output binary is named 'belajargolang' and pointed to the entry point cmd/api/main.go
RUN CGO_ENABLED=0 GOOS=linux go build -o belajargolang ./main.go
...
COPY --from=builder /app/belajargolang .

# Give the non-root user ownership of the binary
RUN chown apps:apps /app/belajargolang
